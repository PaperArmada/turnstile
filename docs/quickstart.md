...
- **The CLI is invoked through the same uvx prefix.** For repeated use, alias
  ...
  ```bash
  alias turnstile='uvx --python 3.12 --from "turnstile-cli @ git+https://github.com/PaperArmada/turnstile.git@v0.1.1#subdirectory=packages/turnstile-cli" turnstile'
  ```
